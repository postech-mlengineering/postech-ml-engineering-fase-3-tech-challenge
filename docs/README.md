...
#### KMeans — Agrupamento de Companhias Aéreas (Dashboard)

Algoritmo de clusterização não supervisionada que agrupa as companhias aéreas em perfis operacionais distintos, sem a necessidade de rótulos pré-definidos. O KMeans minimiza a distância intra-cluster (inércia), posicionando cada companhia no grupo cujo centróide é mais próximo de seus atributos operacionais.

**Como funciona no projeto:**

1. As estatísticas médias de cada companhia aérea são calculadas a partir dos dados históricos de voos
2. Os dados são normalizados via `StandardScaler` para eliminar o efeito de escala entre as features
3. O KMeans é aplicado sobre as features selecionadas, gerando K grupos
4. Os resultados são visualizados em gráficos 2D ou 3D interativos com Plotly

**Features disponíveis para agrupamento:**

| Feature | Descrição |
|---|---|
| `TAXI_OUT` | Tempo médio de manobra na saída do aeroporto (min) |
| `TAXI_IN` | Tempo médio de manobra na chegada ao aeroporto (min) |
| `AIRLINE_DELAY` | Atraso médio causado pela própria companhia (min) |
| `WEATHER_DELAY` | Atraso médio causado por condições climáticas (min) |
| `FLIGHT_VOLUME` | Volume total de voos operados |

**Parâmetros configuráveis no dashboard:**

| Parâmetro | Intervalo | Padrão |
|---|---|---|
| Número de clusters (K) | 2 — 8 | 4 |
| Features para agrupamento | Qualquer combinação das 5 acima | TAXI_OUT, TAXI_IN, AIRLINE_DELAY, WEATHER_DELAY |

O dashboard exibe os perfis gerados com as médias de cada cluster destacadas (verde = menor média, vermelho = maior média), permitindo identificar quais companhias operam com maior eficiência e quais concentram mais atrasos de causa própria.
...
### Modelo Supervisionado — Dashboard

#### Logistic Regression
Modelo linear utilizado para predição em tempo real na interface do dashboard. Treinado sobre uma amostra de 500 mil voos com features básicas (mês, dia da semana, horário, distância e companhia aérea). Foi escolhido pela leveza e velocidade de inferência, priorizando responsividade na UI.
...
### 📍 Análise Geográfica
Mapa de arcos interativo (PyDeck) com as rotas e seus índices de atraso médio, com filtros por companhia aérea e estado de origem. Inclui mapa de calor dos aeroportos mais críticos.

### 🤖 Modelo Supervisionado
Interface de predição em tempo real: informe mês, dia da semana, horário de partida, distância e companhia aérea para receber a probabilidade estimada de atraso do voo. Exibe importância das variáveis do modelo.

### 🔍 Modelo Não Supervisionado
Clusterização interativa das companhias aéreas por perfil operacional. O usuário define o número de clusters e as features para agrupamento e visualiza os resultados em gráficos 2D ou 3D com Plotly.
...
> Os dados brutos (`flights.csv`, `airlines.csv`, `airports.csv`) devem estar em `data/raw/` antes de executar qualquer etapa abaixo.

---

## Como Executar

### Passo 1 — Pipeline de Machine Learning
...
### Passo 2 — Pré-processamento para o Dashboard

Como o `flights.csv` é muito pesado (~600 MB), é necessário executar o script de agregação antes de iniciar o dashboard. Ele gera os arquivos `.parquet` otimizados e treina o modelo leve usado na interface de predição.

```bash
python frontend/scripts/aggregate_data.py
```
...
| Visualização | Plotly | 6.7.0 | Gráficos interativos |
| Visualização | Matplotlib / Seaborn | 3.10.9 / 0.13.2 | Gráficos estáticos e de avaliação |
| Frontend | Streamlit | — | Dashboard interativo |
| Mapas | PyDeck | — | Mapas 3D e de calor |
...
| Formato | PyArrow / Parquet | — | Armazenamento eficiente de dados agregados |
...
│   │   ├── 1_📍_Analise_Geografica.py
│   │   ├── 2_🤖_Modelo_Supervisionado.py
│   │   └── 3_🔍_Modelo_Nao_Supervisionado.py
│   ├── scripts/
│   │   └── aggregate_data.py   # Pré-processamento e modelos para o dashboard
│   ├── utils/
│   │   └── data_loader.py
│   └── requirements.txt
